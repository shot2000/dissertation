...
mod.lm2 <- lm(CarEMTAZ ~ ACRES + AT +TOTAL_EMPL + POP_DENSIT+EMP_DENSIT + EMP_L + EMP_H + AVGWK +AVGPER+AVGAUTO+Avg_TRIPDI+Avg_TRIPSP+Sum_TRIPDU+Avg_TRIPDU, data=data)

mod.lm <- lm(CarEMTAZ ~  ACRES + AT + TOTAL_EMPL + POP_DENSIT + EMP_DENSIT + EMP_M + AVGWK + AVGAUTO + Avg_CarbEM + Avg_TRIPSP, data=data)

##VIF > 10 is highly colinear

vif (mod.lm2)
 ACRES         AT TOTAL_EMPL POP_DENSIT EMP_DENSIT      EMP_L      EMP_H 
      1.67       2.70       1.48       1.53       1.56       1.25       1.20 
     AVGWK     AVGPER    AVGAUTO Avg_TRIPDI Avg_TRIPSP Sum_TRIPDU Avg_TRIPDU 
      3.73       2.37       3.92       5.64       4.65       1.48       7.35 
